    ...
    <div className="min-h-screen bg-studio-950 flex flex-col items-center justify-center p-8">
      <div className="max-w-2xl w-full">
        ...
        <div className="grid gap-4">
          {listOverlay.map((item, index) => (
            <button
              key={index}
              onClick={() => router.push(item.href)}
              className="group w-full text-left p-5 rounded-2xl border border-studio-border bg-studio-900 hover:bg-studio-800 hover:border-white/10 transition-all duration-200"
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-display font-semibold text-base">{item.name}</h3>
