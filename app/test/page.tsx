...
export default function TestPage() {
  ...
  const presets = [
    ...
    { name: 'Maya', amount: 200000, message: 'Keren!' },
  ]
    ...
    <div className="min-h-screen bg-studio-950 flex items-center justify-center p-8">
      <div className="max-w-lg w-full space-y-6">
        <div className="text-center">
          ...
          <p className="text-zinc-500 text-xs mt-1">Kirim test tip ke glass tipjar</p>
        </div>
        ...
        <div className="bg-studio-900 border border-studio-border rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {presets.map(p => (
              <button key={p.name} onClick={() => sendTip(p.name, p.amount, p.message)} className="py-2 px-3 rounded-xl text-xs font-medium border border-studio-border bg-studio-800/50 text-zinc-400 hover:bg-studio-800 hover:text-white hover:border-white/15 transition-all active:scale-[0.95]">
